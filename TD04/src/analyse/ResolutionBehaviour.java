...
public class ResolutionBehaviour extends CyclicBehaviour {
	...
	public void action() {
		...
		if (message != null) {
			...
			try {
				requestMessage = mapper.readValue(message.getContent(), CellsMessage.class);
				...
				for(int i = 1; i<9; i++){
					...
					if(analyseCells[i].numberOfPossible() == 2){
						// on cherche les 2 mêmes chiffres ailleurs
						for(int j = i+1; j<9; j++){
							if(analyseCells[j].numberOfPossible() == 2 
									&& analyseCells[j].getBinaryPossibles() == analyseCells[i].getBinaryPossibles()){
								...
								for(int k = 0; k<9; k++){
									...
									removeList.add(j);
									if(k!=j && k!=i){
										analyseCells[k].removeValsPossibles(removeList);
									}
								}
							}
						}
